...
async function createThreeVersionChain() {
  ...
  const v2 = await service.amendPrescriptionAuthorized(v1.prescription_id, { dosage_value: '650' }, 'PRV-001', 'Pain not controlled');
  const v3 = await service.amendPrescriptionAuthorized(v1.prescription_id, { duration_days: 5 }, 'PRV-001', 'Extend course');
  return [v1, v2, v3];
}
...
describe('3. verifyAnchor on a tampered prescription row', () => {
  test('raw dosage edit → integrityRootMatch false while chainIntact stays true', async () => {
    ...
    // Same pattern as Module 2's tamperDemo: bypass the repository entirely.
    await pool.execute('UPDATE prescription_version SET dosage_value = ? WHERE id = ?', ['5000', clean.id]);
    ...
    expect(hashEngine.verifyIntegrity(row, row.field_hashes, row.salt).tamperedFields).toEqual(['dosage_value']);
  });

  test('consistent rewrite of data + field_hashes + integrity_root is still caught by the anchor', async () => {
    const clean = await repository.createPrescription(RX);
    const forged = { ...clean, dosage_value: '5000.000' };
    const forgedHashes = hashEngine.computeFieldHashes(forged, clean.salt);
    await pool.execute(
      'UPDATE prescription_version SET dosage_value = ?, field_hashes = ?, integrity_root = ? WHERE id = ?',
      ['5000', JSON.stringify(forgedHashes), hashEngine.computeIntegrityRoot(forgedHashes), clean.id],
    );
...
describe('4. rejected amendment attempts produce no ledger entry', () => {
  test.each([
    ['wrong provider', { dosage_value: '5000' }, 'PRV-002', 'NOT_AUTHORIZED_PROVIDER'],
    ['patient_id change', { patient_id: 'PAT-002' }, 'PRV-001', 'INVALID_AMENDMENT_FIELD'],
    ['drug_name change', { drug_name: 'Tramadol' }, 'PRV-001', 'INVALID_AMENDMENT_FIELD'],
  ])('%s', async (_label, changes, providerId, code) => {
    const v1 = await repository.createPrescription(RX);
...
describe('5. every version has exactly one ledger entry, and vice versa', () => {
  test('mixed create / amend / delegated amend / revoke across several prescriptions', async () => {
    ...
    const b = await repository.createPrescription({ ...RX, patient_id: 'PAT-002', provider_id: 'PRV-002', drug_name: 'Atorvastatin', dosage_value: '20', frequency: 'once daily', duration_days: 30, drug_class: 'statin' });
    const c = await repository.createPrescription({ ...RX, patient_id: 'PAT-003', drug_name: 'Metformin', frequency: 'twice daily', duration_days: 30, drug_class: 'biguanide' });
    await pool.execute(
    ...
    await service.amendPrescriptionAuthorized(a.prescription_id, { dosage_value: '650' }, 'PRV-001');
    await service.amendPrescriptionAuthorized(b.prescription_id, { dosage_value: '40' }, 'PRV-001', 'delegate');
    await service.revokePrescription(c.prescription_id, 'PRV-001', 'Switched therapy');
    await service.amendPrescriptionAuthorized(a.prescription_id, { frequency: 'every 8 hours' }, 'PRV-001');
    await service.amendPrescriptionAuthorized(b.prescription_id, { duration_days: 60 }, 'PRV-002');
    ...
    await service.amendPrescriptionAuthorized(c.prescription_id, { duration_days: 9 }, 'PRV-001').catch(() => {});
    await service.amendPrescriptionAuthorized(b.prescription_id, { patient_id: 'PAT-001' }, 'PRV-002').catch(() => {});
...
describe('6. failed version insert leaves no orphaned ledger entry', () => {
  test.each([
    ['createPrescription with unknown patient_id (FK)', async () => repository.createPrescription({ ...RX, patient_id: 'PAT-GHOST' })],
    ['amendPrescription with unknown amended_by_provider_id (FK)', async (existing) => repository.amendPrescription(existing.prescription_id, { duration_days: 9 }, 'PRV-GHOST')],
    ...
    const next = await repository.amendPrescription(existing.prescription_id, { duration_days: 4 });
